...
async function runMigration() {
  try {
    ...
    console.log('Lecture du fichier de migration des demandes de congés...');
    const migrationPath = join(__dirname, '..', 'supabase', 'migrations', '20240327000000_leave_requests.sql');
    ...
    console.log('Migration des demandes de congés exécutée avec succès');
  } catch (error) {
    console.error('Erreur lors de la migration des demandes de congés:', error);
    process.exit(1);
  }
}
